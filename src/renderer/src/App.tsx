  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg)' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '16px 16px 14px', borderBottom: '1px solid var(--divider)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text-900)', lineHeight: 1.2 }}>Collection Hub</div>
        ...
        {/* Footer */}
        <div style={{
          padding: '12px', borderTop: '1px solid var(--divider)',
          display: 'flex', alignItems: 'center', justifyContent: 'space-between'
        }}>
          <span style={{ fontSize: 11, color: 'var(--text-400)' }}>ryelae • 2026</span>
          <button
            onClick={() => setDark((d) => !d)}
            title={dark ? 'Switch to light mode' : 'Switch to dark mode'}
            style={{
              background: 'var(--divider)', border: '1px solid var(--border)',
              borderRadius: 6, padding: '4px 8px', color: 'var(--text-500)',
      ...
      <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        {tab === 'home' && <WelcomeView onGoToSettings={() => setTab('settings')} onGoToImport={() => setTab('import')} />}
